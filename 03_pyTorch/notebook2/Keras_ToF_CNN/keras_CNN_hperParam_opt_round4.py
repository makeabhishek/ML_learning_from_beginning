...
##with open('HyperParamOpt_round4.txt','w') as f:
##	for i in range(np.size(headers)):
##		f.write(str(headers[i])+', ')
##	f.write('\r\n')
##	np.savetxt(f,metrics,fmt='%.5e',delimiter=', ',newline='\r\n')
##
# load metrics/headers from file
with open('HyperParamOpt_round4.txt') as csvfile:
...
metrics=np.array(data[2:-1:2]).astype(float)
...
plt.colorbar()
[plt.text(i,j,'%.2e'%C[i,j],color='red',verticalalignment='center',horizontalalignment='center', fontsize=6) for i in range(np.shape(C)[0]) for j in range(np.shape(C)[1])]
plt.tight_layout()
