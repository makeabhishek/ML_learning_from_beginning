...
# %% Testing the model
modelFile = "c_training_data_Rec_7to11_Din_random_10%_11012024_test1"

testFile = "test_received_waveforms_Din_32.21_Dout_39.03TEL_rand_thickness_rand_3.41_10042023_.mat";
...
# %%
winLen = 100#matlabDat_test['winLen'][0][0] # number of samples per window
timSmpl = 600; # Number of times sample used for testing the data
t = matlabDat_test['t'][0:timSmpl,:]                        # time
testCCE = matlabDat_test['dat'][0:timSmpl,:];               # unknown test data
testLabelsTrue = matlabDat_test['labels'][0:timSmpl,:];     # True values for validation or comparison with prediction.
...
timSmplForNormalize = timSmpl #200

for i in range (N):
    # select the time samples you want to consider to find the max. 
    tt = np.where(testLabels[:,i]==np.amax(testLabels[0:timSmplForNormalize,i]))
    tt= np.array(tt)
    if len(tt[0,:])>0:
